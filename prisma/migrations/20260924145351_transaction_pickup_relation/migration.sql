...
-- CreateEnum
CREATE TYPE "material_units" AS ENUM ('kg', 'g', 'l', 'ml', 'piece');

-- CreateEnum
CREATE TYPE "pickup_statuses" AS ENUM ('pending', 'assigned', 'on_the_way', 'arrived', 'completed', 'cancelled');

-- CreateEnum
CREATE TYPE "payout_statuses" AS ENUM ('pending', 'paid', 'cancelled');
...
-- CreateIndex
CREATE UNIQUE INDEX "wallet_transactions_pickup_id_key" ON "wallet_transactions"("pickup_id");
...
-- AddForeignKey
ALTER TABLE "wallet_transactions" ADD CONSTRAINT "wallet_transactions_pickup_id_fkey" FOREIGN KEY ("pickup_id") REFERENCES "pickups"("id") ON DELETE RESTRICT ON UPDATE CASCADE;
